...
topTen <- varImportances[varImportances > 0.87, ] %>% sort(decreasing = TRUE)
...
```
```{r}
varImpPlot(propModel, n.var = 10, main = "Flipped District Model")
```
```{r}
flippedTree <- rpart(
  flipped ~ .,
...
```{r}
flippedTree2 <- rpart(
  flipped ~ associates + diff_county + disabled,
  method='class',
  data = trainProp
  )
...
```{r}
flippedTree3 <- rpart(
  flipped ~ commute + uninsured + born_abroad,
  method='class',
  data = trainProp
  )
...
```{r}
flippedTree4 <- rpart(
  flipped ~ high_school + born_out_of_state,
  method='class',
  data = trainProp
  )
